		...
		try (BufferedReader openInput = new BufferedReader(
			...
			while (!isEndCallWord(ui)) {
				...
				if (MatcherPatternForUserInput.isConfirmInputOfMenuNumber(ui)) {
					switch (ui) {
					case "1":
						ShowAddressFromZipCode outputFromMenu1 = new ShowAddressFromZipCode();
					...
					case "4":
						...
						if (isEndCallWord(menu4.getIsEndOfMenu4())) {
							break;
						}
						// 担当のクラスを挿入
						AddressBookForMenu4 outputFromMenu4 = new AddressBookForMenu4();
						outputFromMenu4
								.setPersonalInfo(menu4.getPersonalInfo());
						outputFromMenu4.outputPersonalInfoTo01HOKKAI();
						break;
					case "5":
